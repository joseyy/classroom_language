...
Matrix<T> &Matrix<T>::operator*=(const Matrix<T> &obj)
{
    ...
    // Make sure m_this == n_other
    try
    {
        if (n_this != m_other)
        {
            throw "Error: Matrix dimensions are not compatible";
        }
    }
    ...
    this->cudaMemoryManager->allocate(d_result, m_this * n_other);

    this->cudaMemoryManager->copyToDevice(d_mat, this->mat, m_this * n_this);
    this->cudaMemoryManager->copyToDevice(d_mat2, obj.data(), m_other * n_other);

    // Set Blocks, Threads and Grids
    dim3 threadsPerBlock(32, 32);
    dim3 blocksPerGrid((n_other + threadsPerBlock.x - 1) / threadsPerBlock.x,
                       (m_this + threadsPerBlock.y - 1) / threadsPerBlock.y);

    multiply_matrices_wrapper(d_mat,
                              d_mat2,
                              d_result,
                              m_this,
                              m_other,
    ...
    // delete this->mat
    delete[] this->mat;
    // set this matrix size
    this->size[0] = m_this;
    this->size[1] = n_other;
    // Copy result back to host
    this->cudaMemoryManager->copyToHost(this->mat, d_result, m_this * n_other);
    // Free memory
    this->cudaMemoryManager->deallocate(d_mat);
    this->cudaMemoryManager->deallocate(d_mat2);
    this->cudaMemoryManager->deallocate(d_result);
}

template <typename T>
Matrix<T> Matrix<T>::operator*(const Matrix<T> &obj)
{
    // get size of both matrices
    size_t m_this = this->size[0];
    size_t n_this = this->size[1];
    size_t m_other = obj.size[0];
    size_t n_other = obj.size[1];
    ...
    // Copy result back to host
    T *result = nullptr;
    this->cudaMemoryManager->copyToHost(result, d_result, m_this * n_other);
    // Free memory
    this->cudaMemoryManager->deallocate(d_mat);
    this->cudaMemoryManager->deallocate(d_mat2);
    this->cudaMemoryManager->deallocate(d_result);

    size_t size[2] = {m_this, n_other};
    Matrix<T> resultMatrix(result, size);
...
void Matrix<T>::transpose()
{
    ...
    transpose_matrix_wrapper(d_mat, d_result, m, n, blocksPerGrid, threadsPerBlock);
    ...
    // Copy result back to host
    this->cudaMemoryManager->copyToHost(this->mat, d_result, m * n);
